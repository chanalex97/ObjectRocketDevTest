...
class Promotions(object):
    ...
    def is_bogo(self,current_order):
        #make sure that whatever we're passing in as a parameter is actually a cart object
        if isinstance(current_order, Cart):
    ...
    def is_chmk(self, current_order):
        #make sure that whatever we're passing in as a parameter is actually a cart object
        #and check if the discount has already been used, exit the function
        if isinstance(current_order, Cart) and not self.is_chmk_used:
    ...
    def is_apom(self, current_order):
        #make sure that whatever we're passing in as a parameter is actually a cart object
        # and if the discount has already been used, exit the function
        if isinstance(current_order, Cart) and not self.is_apom_used:
            #list to hold product codes
            product_codes = []
            #collect all the product codes and save in product_codes
            for product in current_order.cart:
